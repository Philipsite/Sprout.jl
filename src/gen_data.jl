...
function generate_dataset(n::Int, filename_base::String;
                          ...
                          Xoxides               ::Vector{String}    = ["SiO2"; "Al2O3"; "CaO"; "FeO"; "MgO"; "Na2O"],
                          sys_in                ::String            = "wt",
    ...
    bulks = reduce(hcat, [out_i.bulk_wt for out_i in out])
    ...
    @threads for i in ProgressBar(eachindex(out))
        ...
        # add bulk and shear modulus
        modulus_i = vcat(out[i].bulkMod, out[i].shearMod)
        modulus[:, i] .= modulus_i

        # add ph_vol
        ph_vol_i = out[i].ph_frac_vol
        ph_vol[indices_phaselist, i] .= ph_vol_i
        ...
        ph_comp_i = reduce(vcat, [ss.Comp_wt for ss in out[i].SS_vec])
    ...
    y_names = [(phase_list .* "_vol")..., (phase_list .* "_rho")..., vcat([repeat([ss], 6) .* ("_" .* oxides_in_out)  for ss in SS]...)..., "bulk_modulus", "shear_modulus"]

    x_data = vcat(pressure_kbar', temperature_C', bulks)
    y_data = vcat(ph_vol, ph_ρ, ss_comp, modulus)
